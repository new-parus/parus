...
public class Lexer {
	...
	public String nextToken() throws EOFException{
		...
		switch(state){
			case ST_BEGIN: throw new EOFException();
			case ST_STRING: 
							do {
                                ch = input();
								buf.append(ch);
							...
							return buf.toString();
			case ST_NUMBER: readNumber(); break;
			case ST_WORD:	// read letters, underscores and digits 
							...
							break;
			case ST_DELIM: break; // all in already buf
			case ST_TAG: readTag(); break; //in progress
			default: break; // no such case
		}
		...
		try{
			while(state == ST_BEGIN){
				...
				switch (ch) {
					...
					case '\r':
					case '\t': break;
					case '<': state = ST_TAG; buf.append(ch); break;
					case '(': state = ST_DELIM; buf.append(ch); break;
					case ')': state = ST_DELIM; buf.append(ch); break;
					case '"': state = ST_STRING; buf.append(ch); break;
					case '/': ch = input();
							  if (ch == '*') {
								  skipSlashStarComments();
							  } else if (ch == '/'){
								  skipSlashSlashComments();
							  } else unput(ch); // FIXME - other case 
							  break;
