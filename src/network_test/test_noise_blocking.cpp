...
Test_time_result_type* test_noise_blocking(int mes_length,int num_repeats, int num_noise_repeats, int loading, int num_processors)
{
	...
	/*
	 * Try get enough memory. If didn't, return NULL
	 */ 
		...
		for ( proc1 = 0; proc1 < comm_size; proc1++ )
			for ( proc2 = 0; proc2 < comm_size; proc2++)
			{
				...
				if ( ( comm_rank == proc1 ) || ( comm_rank == proc2 ) )
				{
					...
					if ( comm_rank == proc1 )
					{
						MPI_Send( td.send_data[proc2], mes_length, MPI_BYTE, proc2, 0, MPI_COMM_WORLD );
						MPI_Recv( td.recv_data[proc2], mes_length, MPI_BYTE, proc2, 0, MPI_COMM_WORLD, &status );
						time_end = px_my_cpu_time();
						td.tmp_results[proc2][i] = (px_my_time_type)(time_end - time_beg);
					}
					if ( comm_rank == proc2 )
					{
						MPI_Recv( td.recv_data[proc1], mes_length, MPI_BYTE, proc1, 0, MPI_COMM_WORLD, &status );
						MPI_Send( td.send_data[proc1], mes_length, MPI_BYTE, proc1, 0, MPI_COMM_WORLD );
						time_end = px_my_cpu_time();
